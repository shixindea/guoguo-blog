              ...
              <div className="space-y-4">
                {activeTab === "all" && (
                  <h3 className="font-bold text-slate-900 dark:text-white flex items-center gap-2">
                    <User className="w-4 h-4" /> 相关用户
                  </h3>
                )}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {USERS.map((u) => (
                    <div key={u.id} className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
              ...
              <div className="flex flex-wrap gap-2">
                {["React 19", "Vue 3", "AI", "Rust", "面试题", "架构"].map((item) => (
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 rounded-lg text-sm hover:bg-blue-50 hover:text-blue-600 transition-colors"
                  >
                    {item}
                  </button>
                ))}
